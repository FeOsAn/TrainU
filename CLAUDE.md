...
- **Phase 12 (done)** — the deploy hardened, and an auth gate that was open.

  The first deploy crashed on Node 18 and was fixed blind. Reproducing the
  production shape exactly instead — clean export, `npm ci`, build, then prune
  to production dependencies — found the next crash waiting behind it, and
  several things no local check could ever have seen:
  - **`dist/index.js` could not load without devDependencies.** `server/index.ts`
    only `await import("./vite")` in development, but esbuild inlines a
    dynamically imported file and hoists ITS imports (`vite`,
    `@vitejs/plugin-react`) to the top of the bundle. Development has its own
    entry point now (`server/dev.ts`); the production entry cannot reach vite.
  - **`better-sqlite3` was a devDependency**, surviving only because it happens
    to be drizzle-orm's optional peer.
  - **The auth gate was bypassed by capital letters.** `requireAuth` tested
    `req.path.startsWith("/api")` while Express routes case-insensitively, so
    `/API/goals` skipped the gate and reached the `/api/goals` handler — every
    read and write, including a Garmin sync on stored credentials, open to
    anyone who guessed the URL. It is mounted `app.use("/api", requireAuth)`
    now, matched by the same router as the routes it protects, and
    `auth.test.ts` walks the app's own route table and tries every route in
    three casings.
  - **`/api/health` sat behind that gate** (401), so it could never serve as a
    healthcheck. It is public, and returns 503 for the misconfigurations that
    would otherwise go live looking healthy: no `APP_PASSWORD`, an unreadable
    database, and on Railway a database that is not on the attached volume —
    which works perfectly until the next deploy erases everything.
  - A boot failure inside the async IIFE was swallowed by the
    `unhandledRejection` logger, leaving a process alive and never listening.
  - **The SPA fallback answered a missing `/assets/*.js` with index.html and a
    200** — exactly what every tab open across a redeploy requests next, so a
    MIME error and a blank page.

  So none of it can recur silently: `scripts/build-server.mjs` fails the build
  when the Node running it does not satisfy `engines.node` or when any external
  import is not a builtin or a runtime dependency, and `npm run smoke` builds
  the real artifact, prunes to production dependencies and boots it four times
  — fresh volume, redeploy onto the same volume, no `APP_PASSWORD`, and Railway
  with no volume attached — asserting the last two fail their healthcheck so
  the deployment never swaps in.

  Also: the first proof that a VALID FIT file parses end to end. Every FIT test
  until now fed the parser garbage and checked it failed cleanly; nothing had
  shown the path turns a real file into a session, and the worker it runs in
  never worked outside dev until `adb2305`. `server/fixtures/run-5k.fit` is a
  5 km / 25:00 run encoded with Garmin's own FIT SDK and verified by its
  decoder's integrity check, regenerable byte-for-byte by `make-run-5k.mjs`.
  It is still synthetic — no device wrote it — so the first real Garmin export
  remains the first test against device-specific messages.

  722 tests, `tsc` clean, build green, smoke green.
...
```
npm install
npm run dev     # http://localhost:5000 (server/dev.ts — the only entry that can reach vite)
npm run check   # tsc
npm test        # every *.test.ts under client/, server/, shared/ and scripts/
npm run build   # vite + scripts/build-server.mjs -> dist/
npm start       # production, serves dist/public
npm run smoke   # builds the real artifact, prunes to production deps, boots it
                # four times (fresh volume, redeploy, no APP_PASSWORD, no volume)
```

`npm test` globs `**/*.test.ts` rather than a hand-written list of
directories, because a list is a thing you forget to add to — Phase 11 found
six test files, including every one written for the client in Phase 10, that
had never run once.
...
Point the platform's healthcheck at `/api/health`. It is the one route outside
the auth gate, and it deliberately fails (503) for the misconfigurations that
otherwise deploy looking healthy — no `APP_PASSWORD`, an unreadable database,
or a database that is not on the attached volume, which works perfectly right
up until the next deploy erases the athlete's history.
